            ...
            <div>
                ...
                <div key={booking._id} className='grid grid-cols-1 md:grid-cols-[3fr_2fr_1fr] w-full border-b border-gray-300 py-6 first-border-t'>
                    ...
                    <div className='flex flex-col md:flex-row'>
                        <img src={booking.room?.images[0]} alt="hotel-imf" className='min-md:w-44 rounded shadow object-cover' />
                        <div className='flex flex-col gap-1.5 max-md:mt-3 min-md:ml-4'>
                            <p className='font-player text-2xl'>{booking.hotel?.name}
                                <span className='font-inter text-sm'>({booking.room?.roomType})</span>
                            </p>
                            <div className='flex items-center gap-1 text-sm text-gray-500'>
                                <img src={assets.locationIcon} alt="hotel-imf" className='' />
                                <span>{booking.hotel?.address}</span>
                            </div>

                            <div className='flex items-center gap-1 text-sm text-gray-500'>
                                <img src={assets.guestsIcon} alt="guest" className='' />
                                <span>Guests:{booking.guests}</span>
                            </div>
                            <p className='taxt-base'>Total: ${booking.totalPrice}</p>
                        </div>
                    </div>
                    ...
                    <div className='flex flex-col items-start justify-center pt-3'>
                        ...
                        {
                            !booking.isPaid && (
                                <button onClick={()=>handelPayment(booking._id)} className='px-4 py-1.5 mt-4 text-xs border border-gray-500 rounded-full hover:bg-gray-50 transition-all cursor-pointer'>
                                    Pay Now
                                </button>
                            )
                        }
